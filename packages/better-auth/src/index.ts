...
export type NimiqAuthPluginOptions = Partial<Pick<NimiqAuthOptions, 'nimiqAuthJwtDuration' | 'appName'>>

export function nimiq({ appName, nimiqAuthJwtDuration }: NimiqAuthPluginOptions = {}): BetterAuthPlugin {
  ...
  return {
    ...
    endpoints: {
      ...
      verifyJwt: createAuthEndpoint(
        ...
        async (ctx) => {
          const mod = await import('./index_bg.wasm?module' as string)
          await init(mod)
